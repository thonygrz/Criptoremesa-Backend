...
countryStatesController.getStatesByCountryId = async (req, res, next) => {
  try {
    ...
    if (!req.isAuthenticated() && env.ENVIROMENT === ENVIROMENTS.PRODUCTION){
      ...
      // calling service
      logger.info(`[${context}]: Sending service to get country States by id ${req.params.id_country}`);
      ObjLog.log(`[${context}]: Sending service to get country States by id ${req.params.id_country}`);

      let finalResp = await countryStatesService.getStatesByCountryId(req, res, next);
