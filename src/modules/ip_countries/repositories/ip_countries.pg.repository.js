...
ip_countriesPGRepository.getip_countriesClient = async () => {
  try {
    logger.info(`[${context}]: Getting ip_countries from db`);
    ObjLog.log(`[${context}]: Getting ip_countries from db`);
...
ip_countriesPGRepository.getid_by_nameClient = async (name) => {
  try {
    logger.info(`[${context}]: Getting id from db`);
    ObjLog.log(`[${context}]: Getting id from db`);
    await pool.query("SET SCHEMA 'sec_cust'");
    const resp = await pool.query(
      `SELECT * FROM sec_cust.v_ip_countries_get_id_by_name(${name})`
    );
    return resp.rows;
  } catch (error) {
    throw error;
